...
def _safe_find(collection_name, query=None, projection=None):
    try:
        ...
        print(f"[WARN] _safe_find fallback to CSV: {e}")
        filename = {"users": "users.csv", "resources": "resources.csv", "access_logs": "access_logs.csv"}[
            collection_name]
...
def _init_users():
    try:
        ...
        csv_path = os.path.join(DATASETS, "users.csv")
        if not os.path.exists(csv_path):
            print(f"[WARN] {csv_path} not found, skipping users import")
            return
        ...
        for _, row in df.iterrows():
            ...
            docs.append({
                "user_id": user_id,
                "name": str(row.get("name", user_id)),
                "role": str(row.get("role", "medical")),
                "department": str(row.get("department", "general")),
                "mfa_enabled": str(row.get("mfa_enabled", "false")).lower() == "true",
                "clearance": str(row.get("clearance", "medical")),
                "password_hash": bcrypt.hashpw(user_id.encode(), bcrypt.gensalt()).decode(),
                "totp_secret": pyotp.random_base32(),
                "email": str(row.get("email", "")) if pd.notna(row.get("email")) else None,
                "account_activated": str(row.get("account_activated", "false")).lower() == "true",
            })
        if docs:
            col.insert_many(docs)
            print(f"[INIT] {len(docs)} utilisateurs importés dans 'users'.")
    except Exception as e:
        print(f"[INIT ERROR] Failed to initialize users: {e}")
...
def _init_resources():
    try:
        ...
        csv_path = os.path.join(DATASETS, "resources.csv")
        if not os.path.exists(csv_path):
            print(f"[WARN] {csv_path} not found, skipping resources import")
            return

        docs = pd.read_csv(csv_path).to_dict(orient="records")
        if docs:
            col.insert_many(docs)
            print(f"[INIT] {len(docs)} ressources importées dans 'resources'.")
    except Exception as e:
        print(f"[INIT ERROR] Failed to initialize resources: {e}")
...
def _init_logs():
    try:
        ...
        csv_path = os.path.join(DATASETS, "access_logs.csv")
        if not os.path.exists(csv_path):
            print(f"[WARN] {csv_path} not found, skipping logs import")
            return
        ...
        print(f"[INIT ERROR] Failed to initialize logs: {e}")
...
def login_page():
    if request.method == "POST":
        ...
        if user and _is_account_locked(user):
            ...
            flash(
                f"Compte temporairement verrouillé après trop d'échecs. Réessayez dans {int(LOCK_DURATION.total_seconds() // 60)} minutes.",
                "danger")
            return render_template("login.html"), 423
        # Le mot de passe par défaut (= user_id) n'est un raccourci valide QUE
        # tant que le compte n'a pas été activé.
...
def export_top_users_csv():
    ...
    return Response(output, mimetype="text/csv",
                    headers={"Content-Disposition": "attachment; filename=top_risky_users.csv"})
...
def resource_detail(resource_id):
    ...
    if not resource:
        flash("Ressource introuvable.", "danger")
        return redirect(url_for("resources_page"))
...
def audit_logs_page():
    ...
    denies_by_user = {}
    for l in logs:
        if str(l.get("success")).lower() == "false":
            denies_by_user[l.get("user_id", "?")] = denies_by_user.get(l.get("user_id", "?"), 0) + 1
...
def export_audit_csv():
    ...
    fields = ["timestamp", "user_id", "role", "department", "resource_id", "resource_type", "sensitivity", "action",
              "ip", "success", "mfa_passed", "reason"]
    output = ",".join(fields) + "\n" + "\n".join(
        ",".join(str(log.get(f, "")).replace(",", " ") for f in fields) for log in logs)
...
def initialize_app():
    """Initialise l'application avec toutes les collections MongoDB."""
    ...
    try:
        ...
        print("[MONGODB] L'application continuera en mode CSV fallback")
    ...
    try:
        ...
        _init_logs()
        ensure_indexes()
        print("[INIT] Collections initialisées avec succès")
    except Exception as exc:
        print(f"[INIT WARNING] Erreur lors de l'initialisation des collections: {exc}")
        print("[INIT] Continuation en mode CSV fallback")

    # Créer le compte admin
    try:
        ensure_admin_account()
        print("[ADMIN] Compte admin synchronisé")
    except Exception as exc:
        print(f"[ADMIN WARNING] Impossible de créer/synchroniser le compte admin : {exc}")
